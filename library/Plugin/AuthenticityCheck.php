...
class Plugin_AuthenticityCheck extends Zend_Controller_Plugin_Abstract {
    ...
    public function preDispatch(Zend_Controller_Request_Abstract $request) {
        ...
        $exclusions = array('error' => 'error', 'schema-builder' => 'set-up-project');

        if ((!in_array($request->getControllerName(), array_keys($exclusions))) && (!in_array($request->getActionName(), $exclusions))) {
    ...
    public function getACL($authUser) {
        ...
        if ($acl === FALSE) {
            ...
            if(!($accessPrivileges = $cache->load('access_privileges'))) {
                $accessPrivileges = array('view' => array('index', 'get', 'view'), 'edit' => array('update', 'edit'), 'delete' => array('delete'), 'import' => array('import', 'import-submit'), 'export' => array('export'));
            ...
            foreach ($roles as $roleName => $roleId) {
                ...
                // Everyone should have access to the login page and logout, so,
                $acl->allow($roleName, 'login');
                $acl->allow($roleName, 'dashboard', 'logout');
            }
