...
class schema_build0 {
    ...
    protected function insertDefaultAcl() {
        if ($this->_executionType === 'execute') {
            $sql = "INSERT INTO `acl` (`id`, `user_type_id`, `controller`, `access`, `status`, `created`) VALUES
                    (1, 1, 'dashboard', 'view', 0, '" . date('Y-m-d G:i:s') . "'),
                    (2, 1, 'role', 'view', 0, '" . date('Y-m-d G:i:s') . "'),
                    (3, 1, 'role', 'edit', 0, '" . date('Y-m-d G:i:s') . "'),
                    (4, 1, 'login', 'view', 0, '" . date('Y-m-d G:i:s') . "')";
        } else {
            if (!$this->_model->checkColumnExists('acl', 'id')) {
                return true;
            }

            $sql = "DELETE FROM `acl` WHERE `id` IN (1, 2, 3, 4)";
        }
    ...
    protected function insertDefaultUsers() {
        if ($this->_executionType === 'execute') {
            $sql = "INSERT INTO `users` (`id`, `first_name`, `last_name`, `email`, `login`, `password`, `user_type_id`, `status`, `created`) VALUES
                    (1, 'Rachit', 'Doshi', 'doshi.rachit@gmail.com', 'rachit.d', 'admin', 1, 0, '" . date('Y-m-d G:i:s') . "'),
                    (2, 'Xyz', 'Abc', 'xyz.a@gmail.com', 'xyz.a', '', 2, 0, '" . date('Y-m-d G:i:s') . "');";
        } else {
            if (!$this->_model->checkColumnExists('acl', 'id')) {
                return true;
            }
    ...
    protected function insertDefaultUserTypes() {
        if ($this->_executionType === 'execute') {
            $sql = "INSERT INTO `user_types` (`id`, `name`, `status`, `created`) VALUES
                    (1, 'admin', 0, '" . date('Y-m-d G:i:s') . "'),
                    (2, 'guest', 0, '" . date('Y-m-d G:i:s') . "');";
        } else {
            if (!$this->_model->checkColumnExists('acl', 'id')) {
                return true;
            }
